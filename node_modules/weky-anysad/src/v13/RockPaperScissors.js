...
module.exports = async (options) => {
	...
	if (data.has(options.message.user.id) || data.has(options.opponent.id)) {
		return;
	}
	data.add(options.message.user.id);
	...
	if (
		...
		options.opponent.id === options.message.user.id
	) {
		return;
	}
	...
	const embed = new Discord.MessageEmbed()
		...
		.setDescription(
			options.acceptMessage
				.replace('{{challenger}}', options.message.user.id)
	...
	const question = await options.message.followUp({
		embeds: [embed],
	...
	Collector.on('collect', async (_btn) => {
		...
		if (_btn.customId === 'deny') {
			...
			data.delete(options.message.user.id);
			...
			collector.on('collect', async (button) => {
				if (
					...
					button.member.id !== options.message.user.id
				) {
					return button.followUp({
						content: options.othersMessage.replace(
							'{{author}}',
							`<@${options.message.user.id}> and <@${options.opponent.id}>`,
						),
						ephemeral: true,
					});
				}
				if (button.member.id === options.message.user.id) {
					...
					if (button.customId === id2) {
						...
						if (challengerChose && opponentChose === true) {
							...
							const _embed = new Discord.MessageEmbed()
								...
								.addFields(
									{
										name: options.message.user.username,
