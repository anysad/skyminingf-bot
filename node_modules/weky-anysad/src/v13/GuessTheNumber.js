...
module.exports = async (options) => {
	...
	if (options.publicGame) {
		...
		if (currentGames[options.message.guild.id]) {
			...
			return options.message.followUp({ embeds: [embed] });
		}
		const embed = new Discord.MessageEmbed()
			.setDescription(
		...
		let btn1 = new Discord.MessageButton()
			...
			.setCustomId(id);
		const msg = await options.message.followUp({
			embeds: [embed],
		...
		gameCollector.on('collect', async (button) => {
			if (button.user.id !== options.message.user.id) {
			...
			if (button.customId === id) {
				...
				options.message.inlinefollowUp({
					embeds: [_embed],
				});
			}
		});
		collector.on('end', async (_collected, reason) => {
			...
			if (reason === 'time') {
				...
				return options.message.followUp({ embeds: [_embed] });
			}
		});
	} else {
		if (data.has(options.message.user.id)) return;
		...
		const collector = await options.message.channel.createMessageCollector({
			filter: (m) => m.author.id === options.message.user.id,
			time: options.time,
		});
		...
		collector.on('collect', async (_msg) => {
			if (_msg.author.id !== options.message.user.id) return;
			if (isNaN(_msg.content)) {
				return;
			}
			const parsedNumber = parseInt(_msg.content, 10);
