...
module.exports = async (options) => {
	...
	const think = await options.message.followUp({
		embeds: [
	...
	gameCollector.on('collect', async (trivia) => {
		if (trivia.user.id !== options.message.user.id) {
			return trivia.followUp({
		...
		if (trivia.customId === winningID) {
			...
			options.message.followUp({ embeds: [winEmbed] });
		} else {
			btn1 = new Discord.MessageButton()
				.setEmoji(options.emojis.one)
				.setCustomId(id1)
			...
			options.message.followUp({ embeds: [lostEmbed] });
		}
	});
